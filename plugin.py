...
class BasePlugin:
    ...
    def onStart(self):
        ...
        if self.PHValidx == 0 :
            ...
            self.PHanalyzer = True
        if self.RedoxValidx == 0 :
            self.Redoxanalyzer = False
        else :
            self.Redoxanalyzer = True
        # pump and heater
...
# Heartbeat  ---------------------------------------------------
    ...
    def onHeartbeat(self):
        ...
        if Devices[1].sValue == "0": # Off
                ...
                if self.SPTempCheck >= 31:
                    ...
                    if self.SPTempCheck <= 15:
                        if self.SPTempCheck <= 2:
                            ...
                            self.FiltrationVarNextRefresh = now + timedelta(hours=1)
            ...
            if self.AntiFreeze :
                Devices[11].Update(nValue=0, sValue="Auto - Protection Antigel - Eau à : {}ºC".format(self.SPTemp))
            else :
...
# Read Analyzer---------------------------------------------------------------------------------------------
    def readAnalyzer(self):
        ...
        # Redox
            ...
            self.RedoxVal = 0
            Devices[10].Update(nValue=0, sValue=str(self.PHVal))
            Devices[9].Update(nValue=0, sValue="Waiting values")
        return noerror
