...
<plugin key="SPControl" name="ZZ - Swimming Pool Control with analyzer" author="Ronelabs Team" version="0.1.1" externallink="https://github.com/Erwanweb/SPControl">
    <params>
        ...
        <param field="Mode2" label="filtration power" width="300px">
            <options>
                <option label="Overpowered" value="0.75"/>
                <option label="Normal" value="1"  default="true"/>
                <option label="Underpowered" value="1.25"/>
            </options>
        <param field="Mode3" label="Actuators (0 for not): Filtration, Heater (csv list of idx)" width="200px" required="true" default="0,0,"/>
        <param field="Mode4" label="Permament Water Temp. Sensors (csv list of idx)" width="100px" required="true" default="0"/>
        <param field="Mode5" label="In Flow Water Temp. Sensors (csv list of idx)" width="100px" required="flase" default="0"/>
...
class BasePlugin:
    ...
    def onStart(self):
        ...
        # updating temp and timers
        self.readTemps()
        now = datetime.now()
        self.FiltrationVarNextRefresh = now + timedelta(hours=24)
