...
# Heartbeat  ---------------------------------------------------
    ...
    def onHeartbeat(self):
        ...
        if Devices[1].sValue == "0": # Off
            ...
            #Devices[11].Update(nValue=0, sValue="Auto - Calcul. {}H/24 - {} / {}".format(self.PumpTempRounded,self.startpump,self.stoppump)
...
# Read Analyzer---------------------------------------------------------------------------------------------
    def readAnalyzer(self):
        ...
        self.nextAnalyzer = datetime.now()
        # PH
        jsonData = DomoticzAPI("/json.htm?type=command&param=getdevices&rid={}".format(self.PHValidx))
        if jsonData:
            self.PHVal = jsonData['Data']
            Devices[8].Update(nValue=0, sValue=str(self.PHVal))  # update the dummy device
            Devices[7].Update(nValue=2, sValue="Parfait")
        # Redox
        jsonData = DomoticzAPI("/json.htm?type=command&param=getdevices&rid={}".format(self.RedoxValidx))
        if jsonData:
            self.RedoxVal = jsonData['Data']
            Devices[10].Update(nValue=0, sValue=str(self.RedoxVal))  # update the dummy device
            Devices[9].Update(nValue=2, sValue="Parfait")
